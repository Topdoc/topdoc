...
test('Should be able to parse multiple css files in parallel.', async t => {
  ...
  const buttonResult = await postcss([topdoc()]).process(button, {
    from: './test/fixtures/button.css',
  });
  const selectResult = await postcss([topdoc()]).process(select, {
    from: './test/fixtures/select.css',
  });
  const result = [buttonResult.topdoc.filename, selectResult.topdoc.filename];
  const expected = ['button.css', 'select.css'];
  return t.deepEqual(result, expected);
});

test('Should include nodes with the correct options.', t => {
  const input = read('./test/fixtures/button.css');
  return postcss([topdoc({includeNodes: true})])
    .process(input, {from: './test/fixtures/button.css'})
    .then(result => {
