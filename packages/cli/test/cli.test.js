...
test.cb('should write basic docs import single file', t => {
  ...
  const expected = read(
    path.resolve(__dirname, 'expected', 'button.index.html')
  );
  nixt()
    .cwd(path.resolve(__dirname))
    .expect(() => {
...
test.cb('should copy assets import absolute directory path', t => {
  ...
  nixt()
    ...
    .run(
      `node ../bin/index.js ${source} -d ${destination} -a ${assetDirectory}`
    )
    .end(t.end);
});
...
test.cb('should copy assets import absolute package path', t => {
  ...
  nixt()
    .cwd(path.resolve(__dirname))
    .expect(() => {
      const anotherFile = read(
        path.resolve(destination, 'assets', 'css', 'another.css')
      );
      t.regex(anotherFile, /content: 'yup';/);
    })
    .run(
...
test.cb('should ignore assets when specified', t => {
  ...
  nixt()
    ...
    .run(
      `node ../bin/index.js ${source} -d ${destination} -a ${assetDirectory} -i index.js`
    )
    .end(t.end);
});
...
test.cb('should ignore absolute assets when specified', t => {
  ...
  nixt()
    ...
    .run(
      `node ../bin/index.js ${source} -d ${destination} -a ${assetDirectory} -i ${ignore}`
    )
    .end(t.end);
});
...
test.cb('should output json stdout upon request', t => {
  ...
  const expected = JSON.parse(
    read(path.resolve(__dirname, 'expected', 'button.topdoc.json'))
  );
  nixt()
    .cwd(path.resolve(__dirname))
    .expect(results => {
