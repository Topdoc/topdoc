...
(function() {
  ...
  Topdoc = (function() {
    ...
    Topdoc.prototype.writeJSONFiles = function(callback) {
      if(this.writeJSON) {
          var jsonPath = path.join(this.destination, "topdoc-data.json");
          fs.unlink(jsonPath, null);
          var jsonOutput = JSON.stringify(this.results);
          fs.writeFileSync(jsonPath, jsonOutput);
          callback(null, 'JSON files written');
      }
    };
